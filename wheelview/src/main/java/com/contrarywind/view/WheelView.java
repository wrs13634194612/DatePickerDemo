...
public class WheelView extends View {
    ...
    private int labelTextSize;//选项的文字大小
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        labelTextSize = getResources().getDimensionPixelSize(R.dimen.pickerview_label_textsize);//默认大小
        ...
        if (attrs != null) {
            ...
            textColorCenter = Color.parseColor("#0091FF");
    ...
    private void initPaints() {
        ...
        paintLableText.setColor(Color.parseColor("#0091FF"));
        paintLableText.setAntiAlias(true);
        paintLableText.setTextScaleX(1.1F);
        paintLableText.setTypeface(typeface);
        paintLableText.setTextSize(labelTextSize);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(label) && isCenterLabel) {
         /*   String content = getContentText(visibles[visibles.length / 2]);
            ...
            canvas.drawText(label, drawCenterContentStart + width + textWidth - 8, centerY - 6, paintLableText);*/
...
//                canvas.scale(1.0F, (float) Math.sin(radian));
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    if (!TextUtils.isEmpty(label)) {
                        canvas.drawText(label, drawCenterContentStart + paintCenterText.measureText(contentText) + 8, CENTER_CONTENT_OFFSET, paintLableText);
                    }
                    //设置选中项
